...
def export_requirements(service_path: str, requirements: Tuple[str]) -> None:
    ...
    for requirement in requirements:
        path_here = os.path.join(requirements_here, requirement)
        path_service = os.path.join(requirements_service, requirement)
...
if __name__ == "__main__":
    ...
    for service, path in settings.COMMON_PATHS:
        export_requirements(
            path, (settings.MINI_REQUIREMENTS, settings.COMMON_REQUIREMENTS)
        )
